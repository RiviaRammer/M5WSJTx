...
static void* monitor_alloc(size_t size)
{
#if defined(ESP_PLATFORM)
...
static void* monitor_calloc(size_t count, size_t size)
{
    size_t total = count * size;
    void* ptr = monitor_alloc(total);
    if (ptr != NULL)
        memset(ptr, 0, total);
    return ptr;
}
...
static void waterfall_init(ftx_waterfall_t* me, int max_blocks, int num_bins, int time_osr, int freq_osr)
{
    ...
    me->mag = (WF_ELEM_T*)monitor_alloc(mag_size);
...
void monitor_init(monitor_t* me, const monitor_config_t* cfg)
{
    ...
    me->window = (float*)monitor_alloc(me->nfft * sizeof(me->window[0]));
    if (me->window == NULL)
        return;
    ...
    me->last_frame = (float*)monitor_calloc(me->nfft, sizeof(me->last_frame[0]));
    me->fft_timedata = (kiss_fft_scalar*)monitor_alloc(me->nfft * sizeof(me->fft_timedata[0]));
    me->fft_freqdata = (kiss_fft_cpx*)monitor_alloc((me->nfft / 2 + 1) * sizeof(me->fft_freqdata[0]));
    ...
    me->fft_work = monitor_alloc(fft_work_size);
    if (me->fft_work == NULL)
        return;
    ...
    me->ifft_work = monitor_alloc(ifft_work_size);
...
void monitor_free(monitor_t* me)
{
    ...
    free(me->fft_work);
    free(me->fft_freqdata);
    free(me->fft_timedata);
    free(me->last_frame);
    free(me->window);
}
...
void monitor_process(monitor_t* me, const float* frame)
{
    ...
    if (me->wf.num_blocks >= me->wf.max_blocks)
        return;
    if ((me->fft_timedata == NULL) || (me->fft_freqdata == NULL))
        return;
    ...
    for (int time_sub = 0; time_sub < me->wf.time_osr; ++time_sub)
    {
        ...
        {
            me->fft_timedata[pos] = me->window[pos] * me->last_frame[pos];
        }
        kiss_fftr(me->fft_cfg, me->fft_timedata, me->fft_freqdata);
        ...
        for (int freq_sub = 0; freq_sub < me->wf.freq_osr; ++freq_sub)
        {
            for (int bin = me->min_bin; bin < me->max_bin; ++bin)
            {
                ...
                float mag2 = (me->fft_freqdata[src_bin].i * me->fft_freqdata[src_bin].i) + (me->fft_freqdata[src_bin].r * me->fft_freqdata[src_bin].r);
                float db = 10.0f * log10f(1E-12f + mag2);

#ifdef WATERFALL_USE_PHASE
                // Save the magnitude in dB and phase in radians
                float phase = atan2f(me->fft_freqdata[src_bin].i, me->fft_freqdata[src_bin].r);
